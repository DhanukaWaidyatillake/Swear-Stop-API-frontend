...
export default function PoliciesAndTermsPopup({visible, setVisible, data}) {
    ...
    const renderContent = (text) => {
        ...
        const phoneRegex = /(\+?\d{1,4}[\s.-]?\(?\d+\)?[\s.-]?\d+[\s.-]?\d+)/g;
    ...
    return (
        <div data-dialog-backdrop="manage-list-dialog" data-dialog-backdrop-close="true" onClick={(event) => {
            ...
            <div data-dialog="sign-in-dialog"
                ...
                <div className="min-w-[90%] self-center h-fit overflow-y-auto mt-5 mb-2">
                    <div className="ml-7 mr-4 rounded-md">
                        ...
                        {data.sections.map(({title, content}, index) => (
                            <div key={`section-${index}`} className={`text-left mt-3 ${index === data.sections.length - 1 ? 'pb-7' : ''}`}>
                                    ...
                                    {content.map((item, subIndex) => (
                                        <li key={`item-${index}-${subIndex}`} className="pb-1 pt-2">
                                            {typeof item === "string" ? (
                                                ...
                                                <p ><span className="font-bold text-xl">•</span> {renderContent(item)}</p>
                                            ) : (
                                                Array.isArray(item) ? (
                                                    <div key={`array-${index}-${subIndex}`} className="pl-5">
                                                        {item.map((detail, detailIndex) => (
                                                            <p key={`detail-${index}-${subIndex}-${detailIndex}`}><span className="font-bold pr-1">*</span>{renderContent(detail)}</p>
                                                        ))}
                                                    </div>
                                                ) : (
                                                    ...
                                                    <div key={`nested-${index}-${subIndex}`}>
                                                        ...
                                                        <ul>
                                                            {item.details.map((detail, detailIndex) => (
                                                                <li key={`nested-detail-${index}-${subIndex}-${detailIndex}`} className="pl-5">
                                                                    <span className="font-bold text-xl">•</span> {renderContent(detail)}
                                                                </li>
                                                            ))}
                                                        </ul>
                                                    </div>
